...
init.dis.prop <- c(0.4, 0.59, 0.0, 0.01, 0.0) # ensure sums to 1
sum(init.dis.prop)
...
pr.ret.S <- 1 # probability of returning to the susceptible state after recovery (assuming those not returning to susceptible are pre-susceptible)
...
lnP <- length(nP)
n.perm.P <- ceiling(lnP*prob.never.S)
...
P.dur.tim <- ceiling(runif(lnP2,min.tim.b.ind.S, P.max.tim))
...
lnI <- length(nI)
if (ceiling(prop.I.remain*lnI) > 0) {
    perm.I <- sort(sample(nI,ceiling(prop.I.remain*lnI), replace=F))
...
lnS <- length(nS)
nSexp <- ceiling(lnI * contact.rate * lnS) # number of susceptibles exposed
nE.int <- ceiling(nSexp * daily.trans.pr) # new exposed from internal exposure
nE.ext <- ceiling(lnS * out.dis.contact.rate * out.dis.pr.trans) # new exposed from outside source
...
E1.dur1 <- ceiling(runif(nE.new, min.lat.inc.E, max.lat.inc.E))
...
I.dur.tim1 <- ceiling(runif(lnI2, min.tim.rem.I, max.tim.rem.I))
...
nRpt <- ceiling(nI.tot*pr.R) # number of potential recovereds
if (nRpt > 0) {
  ...
  for (r in 1:nRpt) {
    ...
    R.start[r] <- R.min.start[r] + ceiling(runif(1, R.min.start[r], dim(proj.dat)[2])) # actual day of recovery
  ...
  R.dur <- ceiling(runif(nRpt, min.tim.res, max.tim.res)) # duration of recovered time between min and max
  ...
  nR.perm <- ceiling(dim(R.remain.nonzero)[1]*pr.R.perm.imm)
  ...
  if (length(R.end.before > 0)) {
    ...
    S.return <- sort(sample(endR.before[,1], ceiling(dim(endR.before)[1]*pr.ret.S))) # which individuals become susceptible?
  ...
  if (length(P.return) > 0) {
    P.new.dur <- ceiling(runif(length(P.return), min.tim.b.ind.S, max.tim.b.ind.S))
    for (p in 1:length(P.return)) {
  ...
  for (s in 1:dim(proj.dat2)[1]) {
    ...
    if (is.na(start.day) == F) {
      ...
      for (d in 1:d.len) {
        ...
        nSexp.onday <- ceiling(ceiling(nI.onday * contact.rate) * (nS.onday/pop.size))
        pr.this.ind.E <- (nSexp.onday/nS.onday * daily.trans.pr) + (nS.onday * out.dis.contact.rate * out.dis.pr.trans) # probability this individual is exposed
        state.this.ind <- ifelse((rbinom(1,1,pr.this.ind.E)) == 1, "E", "S") # is this individual exposed?  
        if (state.this.ind == "S") {
          proj.dat[ind.id, today] <- "S"} # end if
        
        if (state.this.ind == "E") {
          E1.dur1 <- ceiling(runif(1, min.lat.inc.E, max.lat.inc.E))
...
if (nRpt > 0) {
  ...
  plot(1:(days.proj+1), TnR, type="l", xlab="days", ylab="proportion recovered")} # end if
par(mfrow=c(1,1))
